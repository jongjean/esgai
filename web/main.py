...
async def save_lead(data: dict):
    logger.info(f"Relaying lead saving request to engine: {data.get('email')}")
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            response = await client.post(f"{ENGINE_URL}/api/leads", json=data)
            response.raise_for_status()
            logger.info(f"Lead saved successfully for {data.get('email')}")
            return response.json()
        except Exception as e:
